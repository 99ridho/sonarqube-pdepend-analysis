...
**PDépend Analyzer (Section 5.1 & 5.2 - Complexity)**
```bash
...
**SonarQube Analyzer (Section 5.3 & 5.5 - Quality Issues)**
```bash
...
2. `generate_section_5_1_descriptive_stats()` - Aggregate statistics (LOC, WMC, CCN averages)
3. `generate_section_5_2_complexity_analysis()` - File-level complexity merging class/method metrics
...
3. `generate_section_5_3_analysis()` - Aggregates issues by quality/severity
4. `create_section_5_3_visualizations()` - Generates 3 figures (stacked bars, pie charts)
5. `create_section_5_5_visualizations()` - Generates 1 figure (hotspot categories)
